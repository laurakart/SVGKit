...
+(Element*) privateGetElementById:(NSString*) idValue childrenOfElement:(Node*) parent
{
	...
	if( [parent isKindOfClass:[Element class]] )
	{
		...
		{
			NSLog(@"parent <%@ id='%@'..> does not match id='%@'", parentAsElement.nodeName, [parentAsElement getAttribute:@"id"], idValue );
			NSLog(@"parent <%@ id='%@'..> has %li child nodes = %@", parentAsElement.nodeName, [parentAsElement getAttribute:@"id"], parent.childNodes.length, parent.childNodes );
		}
#endif
	}
